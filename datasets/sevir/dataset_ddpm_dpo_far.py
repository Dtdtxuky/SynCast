...
class sevir_sproject(Dataset):
    def __init__(self, split, metric = 'csi', model = 'ddim', input_length=6, pred_length=18, data_dir='radar:s3://weather_radar_datasets/sevir', base_freq='5min', height=384, width=384, **kwargs):
        super().__init__()
        ...
        self.file_list = self._init_file_list(split, metric, model)
    ...
    def _init_file_list(self, split, metric, model):
        if split == 'train':
            print('train')
            ...
            print('metric:', metric)
            print('model:', model)
            txt_path = '/mnt/petrelfs/xukaiyi/CodeSpace/rankcast/test/ddpm_ddim_lose_far_sample.txt'
        elif split == 'test':
            print('test')
