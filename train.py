...
def subprocess_fn(args):
    ...
    logger = get_logger("train", args.run_dir, utils.get_rank(), filename='iter.log', resume=True)
    ...
    valid_dataloader = builder.get_dataloader(split = 'val')
    ...
    if args.resume_checkpoint is not None:
        model.load_checkpoint(args.resume_checkpoint, load_optimizer=False, load_scheduler=False)
        print('load from :', args.resume_checkpoint)
        
    else:
        print('not load model from .bash')
...
def main(args):
    ...
    #             print(name)
    ###############################################
    alg_dir = args.cfg.split("/")[-2]
    dataset_dir = args.cfg.split("/")[-1].split(".")[0]
    # cur_dir = os.path.abspath(__file__).split('/')[-2]
    args.outdir = args.outdir  # + "/" + cur_dir
    ## check if outdir exists ##
    if not os.path.exists(args.outdir):
        # raise ValueError(f"outdir {args.outdir} not exists, should link a oss path")
        print('{} does not exist, make a dir'.format(args.outdir))
        os.makedirs(args.outdir, exist_ok=True)
    args.outdir = args.outdir + "/"  + dataset_dir + "/" + alg_dir
    run_dir = os.path.join(args.outdir, f'{desc}')
    relative_checkpoint_dir = dataset_dir + "/" + alg_dir + "/" + f'{desc}'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--outdir',                     type = str,     default='',  help = 'Where to save the results')
    parser.add_argument('--cfg', '-c',                  type = str,     default = '',      help = 'path to the configuration file')
