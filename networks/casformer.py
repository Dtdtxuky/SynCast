...
if __name__ == "__main__":
    ...
    config = dict(
        input_size=16,
        in_channels=8,       # x 和 cond 拼接后就是 8 通道
        mlp_ratio=4.0,
        learn_sigma=False,
        out_channels=72,
        split_num=18,
    ...
    # 创建模型
    model = CasFormer(arch="DiT-custom", config=config)

    # 模拟输入
    b, t, c, h, w = 2, 18, 4, 16, 16  # 注意这里 c=4，因为 x 和 cond 会 concat 成 8
    x = torch.randn(b, t, c, h, w)     # 主输入
    cond = torch.randn(b, t, c, h, w)  # 条件输入
    timesteps = torch.randint(0, 1000, (b,))  # diffusion step
    ...
    # 期望输出: (b, t, out_channels_per_frame, H, W)
