if __name__ == "__main__":
    import sys
    sys.path.append('/mnt/petrelfs/xukaiyi/CodeSpace/rankcast/')

import torch
from torch import nn
from einops import rearrange

from networks.prediff.models.time_embed import TimeEmbedLayer, TimeEmbedResBlock
from networks.prediff.models.cuboid_transformer.cuboid_transformer import PosEmbed, Upsample3DLayer, PatchMerging3D, StackCuboidSelfAttentionBlock
from networks.prediff.models.cuboid_transformer.cuboid_transformer_patterns import CuboidSelfAttentionPatterns
from networks.prediff.models.utils import timestep_embedding, apply_initialization, round_to
...
class prediffNet(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.net = CuboidTransformerUNet(**config)
    ...
    def forward(self, x, timesteps, cond, **kwargs):
        ...
        cond = rearrange(cond, 'b t c h w -> b t h w c')
        out = self.net(x=x, t=timesteps, cond=cond)
...
if __name__ == "__main__":
    ...
    print('inp:', input_data.shape, 'cond:', cond.shape, 'traget:', target.shape)
    
    
    t = torch.randint(0, 1000, (b,)).to(device)
    print('load yaml from config')
    import yaml
    cfg_path = '/mnt/petrelfs/zhouzhiwang/paper_exper/mmrl/prediff_code/ldm_prediff_pred.yaml'
    with open(cfg_path, 'r') as cfg_file:
      cfg_params = yaml.load(cfg_file, Loader = yaml.FullLoader)
    backbone_kwargs = cfg_params['model']['params']['sub_model']['prediffNet']
    # backbone_kwargs = {'input_shape': [inp_length, h, w, c], 'target_shape': [pred_length, h, w, c], 'base_units': 256, 'scale_alpha': 1.0, 'num_heads': 4, 'attn_drop': 0.1,
    #                     'proj_drop': 0.1, 'ffn_drop': 0.1, 'downsample': 2, 'downsample_type': 'patch_merge', 'upsample_type': 'upsample', 'upsample_kernel_size': 3, 
    #                     'depth': [4, 4], 'num_global_vectors': 0, 
    #                      'use_global_vector_ffn': False, 'use_global_self_attn': True, 'separate_global_qkv': True, 'global_dim_ratio': 1,
    #                     'ffn_activation': 'gelu', 'gated_ffn': False, 'norm_layer': 'layer_norm', 'padding_type': 'zeros', 'pos_embed_type': 't+h+w', 'checkpoint_level': 0,
    #                     'use_relative_pos': True, 'self_attn_use_final_proj': True, 'attn_linear_init_mode': '0', 'ffn_linear_init_mode': '0', 'ffn2_linear_init_mode': '2',
    #                     'attn_proj_linear_init_mode': '2', 'conv_init_mode': '0', 'global_proj_linear_init_mode': '2', 'norm_init_mode': '0',
    #                     'time_embed_channels_mult': 4, 'time_embed_use_scale_shift_norm': False, 'time_embed_dropout': 0.0, 'unet_res_connect': True,
    #                     ## TODO: solely determine
    #                     'block_attn_patterns':['axial', 'axial'],
    #                     'down_linear_init_mode': '0', 
    #                     #'use_dec_self_global': False, 'dec_self_update_global': True, 'use_dec_cross_global': False,  'down_up_linear_init_mode': '0',
    #                     }
    model = prediffNet(backbone_kwargs)
    ...
    with torch.no_grad():
        for i in range(5):
            start.record()
            pred = model(x=input_data, timesteps=t, cond=cond) #model(x=input_data, timesteps=t, cond=cond)
