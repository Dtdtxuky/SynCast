...
class checkpoint_ceph(object):
    def __init__(self, conf_path="/mnt/shared-storage-user/xukaiyi/petrel-oss-python-sdk/petreloss.conf", checkpoint_dir="weatherbench:s3://weatherbench/checkpoint") -> None:
        self.client = Client(conf_path=conf_path)
        self.checkpoint_dir = checkpoint_dir

    def load_checkpoint(self, url, abs_path=False):
        if not abs_path:
