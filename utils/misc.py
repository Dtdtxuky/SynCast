...
def init_distributed_mode(args):
    ...
    print(">>> 调试信息: 分布式相关环境变量 <<<")
    for key in ["RANK", "WORLD_SIZE", "LOCAL_RANK", "MASTER_ADDR", "MASTER_PORT"]:
        print(f"{key} = {os.environ.get(key, 'NotSet')}")
    print(">>> ============================ <<<")
    ...
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        ...
        args.init_method = f"tcp://{ip_addr}:{port}"
    else:
        print("Not using distributed mode")
        args.distributed = False
        return
    ...
    print(f"| distributed init (rank {args.rank}, local_rank {args.local_rank}): {args.init_method}", flush=True)
