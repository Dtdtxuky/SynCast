...
class ConfigBuilder(object):
    ...
    def __init__(self, **params):
        ...
        self.guidance = params.get('guidance', 0)
        self.logger = params.get('logger', None)

        self.logger.info(f'##############################self.guidance:{self.guidance}#######################################')
    def get_model(self, model_params = None):
        ...
        if type == 'ddpm_dpo':
            from models.ddpm_dpo import ddpm_dpo
            model = ddpm_dpo(self.logger, **params)
            print('---------------type--------------', type)
        elif type == 'ddpm_diffusion_model':
            from models.diffusion_model import diffusion_model
            model = diffusion_model(self.logger, **params)
            print('---------------type--------------', type)
        elif type == 'spo':
            from models.ddim_spo import spo
            model = spo(self.logger, **params)
            print('---------------type--------------', type)
    ...
    def get_dataset(self, dataset_params = None, split = 'train', guidance=0):
        """
        ...
        if type(dataset_params) == dict:
            dataset_type = dataset_params.get('type', 'fourcastceph')
            # no latent sevir-3h-freq-10min dataset
            if dataset_type == 'sevir_3h':
                from datasets.sevir.dataset_128_10m_3h import get_sevir_dataset
                dataset = get_sevir_dataset(split=split, **dataset_params)
                print(f'{dataset_type}--------------------------len dataset---------------------------------', len(dataset))
                
            elif dataset_type == 'meteonet':
                from datasets.meteonet.dataset import meteonet_24
                dataset = meteonet_24(split=split, **dataset_params)
                print(f'{dataset_type}--------------------------len dataset---------------------------------', len(dataset))
                
            # train meteonet dpo
            elif dataset_type == 'meteonet_dpo':
                from datasets.meteonet.dataset_dpo import meteonet_24
                dataset = meteonet_24(split=split, **dataset_params)
                print(f'{dataset_type}--------------------------len dataset---------------------------------', len(dataset))
                
            # train normal hko7  
            elif dataset_type == 'hko7':
                from datasets.hko7.dataset import data_hko
                dataset = data_hko(split=split, **dataset_params)
                print(f'{dataset_type}--------------------------len dataset---------------------------------', len(dataset))
                
            elif dataset_type == 'hko7_dpo':
                from datasets.hko7.dataset_dpo import data_hko
                dataset = data_hko(split=split, **dataset_params)
                print(f'{dataset_type}--------------------------len dataset---------------------------------', len(dataset))
            
            
                
            elif dataset_type == 'sevir_3h_ddpm_dpo_far':
                from datasets.sevir.dataset_ddpm_dpo_far import get_sevir_dataset
                dataset = get_sevir_dataset(split=split, metric = 'far', model = 'ddim',**dataset_params)
                print('--------------------------len dataset---------------------------------', len(dataset))
                print('--------------------------dataset_type---------------------------------', dataset_type)

            elif dataset_type == 'sevir_3h_ddpm_dpo_spo':
                from datasets.sevir.dataset_ddpm_spo import get_sevir_dataset
                dataset = get_sevir_dataset(split=split, metric = 'far', model = 'ddim', guidance = guidance, **dataset_params)
                print('--------------------------len dataset---------------------------------', len(dataset))
                print('--------------------------dataset_type---------------------------------', dataset_type)
    ...
    def get_dataloader(self, dataset_params = None, split = 'train', batch_size = None, dataloader_params = None, sampler_params = None):
        ...
        dataset = self.get_dataset(dataset_params, split, self.guidance)
        if dataset is None:
            return None
        if sampler_params == None:
            sampler_params = self.sampler_params
        
        dataloader_type = dataloader_params.get('type', 'DataLoader')
        if dataloader_type == 'MultiDatasetDataloader' and split == 'valid':
            ### only use multidatsetdataloader during training ###
            from utils.dataloaders import MultiDatasetDataloader
            return MultiDatasetDataloader(
                concat_dataset=dataset, split=split, batch_size_per_gpu=batch_size, 
                dataloader_params=dataloader_params['kwargs'], sampler_params=sampler_params,
                )
        else:
