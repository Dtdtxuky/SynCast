...
class ddpm_dpo(basemodel):
    def __init__(self, logger, **params) -> None:
        ...
        self.noise_scale = self.noise_scheduler_kwargs.get('noise_scale', 1.0)  ## important: scale the noise to get a reasonable noise process ##
    ...
    def init_ref_model(self):
        main_key = list(self.model.keys())[0]                       
        ref_model = copy.deepcopy(self.model[main_key])        
    ...
    def data_preprocess(self, data):
        ...
        data_dict.update({'inputs': inp_data, 'data_samples': tar_data, 'win_data': win_data, 'lose_data': lose_data, 'file_name': file_name})
        return data_dict
    ...
    def train_one_step(self, batch_data, step):
        ...
        with torch.no_grad():
            ref_noise_pred = self.model[list(self.model.keys())[1]](noisy_tar_dpo, timesteps_dpo, **model_kwargs)
       
        # compute loss ##
        prob_loss_dict = self.loss_dpo(noise_pred, ref_noise_pred, noise_dpo, timesteps) ## important: rescale the loss
        w_loss, l_loss, wt, ref_w_loss, ref_l_loss = prob_loss_dict['w_loss'], prob_loss_dict['l_loss'], prob_loss_dict['w_t'], prob_loss_dict['ref_w_loss'], prob_loss_dict['ref_l_loss']
        model_diff = w_loss - l_loss
        model_w_loss = w_loss.mean()
        model_l_loss = l_loss.mean()
        ref_model_w_loss = ref_w_loss.mean()
        ref_model_l_loss = ref_l_loss.mean()
        ref_model_diff = ref_w_loss - ref_l_loss
        
        loss = model_diff-ref_model_diff
        # print('model_diff-ref_model_diff', loss)
        # print('beta_dpo', self.beta_dpo)
        difloss = loss
        loss = -torch.mean(F.logsigmoid(-self.beta_dpo*wt*loss))
        # print('loss', loss)
        ...
        if (step) % 1 == 0 and (step) % 100 != 0:
            ...
            pred_csi=metrics_pred['avg']['csi']
            pred_far=metrics_pred['avg']['far']
            ...
            ref_csi=metrics_pred_ref['avg']['csi']
            ref_far=metrics_pred_ref['avg']['far']
            ...
            # self.visualizer.save_pixel_image_rl_3(pred=z_sample_prediction, win_image=win, lose_image=lose, target_img = tar, target_ref = z_sample_prediction_ref, step = step, file_name=file_name,
            #                                       metrics_win = metrics_win, metrics_lose=metrics_lose, metrics_pred=metrics_pred, metrics_pred_ref=metrics_pred_ref)
            self.visualizer.save_pixel_image_4(pred_image=z_sample_prediction, target_img=tar, ref=z_sample_prediction_ref, step=step, batchidx=0,
                                               predcsi=pred_csi, predfar=pred_far, refcsi=ref_csi, reffar=ref_far)
        # elif step % 100 == 0:
        #     z_sample_prediction = self.denoise(template_data=tar, cond_data=inp, bs=bs)
        #     z_sample_prediction_ref = self.denoise_ref(template_data=tar, cond_data=inp, bs=bs)
        #     # self.visualizer.save_pixel_image_rl_2(pred=z_sample_prediction, win_image=win, lose_image=lose, target_img = tar, target_ref = z_sample_prediction_ref, step = step, file_name=file_name)
        ...
        #                                        predcsi=pred_csi, predfar=pred_far, refcsi=ref_csi, reffar=ref_far)
            ...
            pass
        return {'model_w_loss': model_w_loss.item(), 'model_l_loss': model_l_loss.item(), 'ref_model_w_loss': ref_model_w_loss.item(), 'ref_model_l_loss': ref_model_l_loss.item(), 'total_loss': loss.item(), 'inter': inter.item()}
    

    def train_spin(self, batch_data, step):
        print("step", step)
        ...
        ## 生成lose sample
        lose = self.denoise_ref(template_data=tar, cond_data=inp, bs=b)
        ## dpo_input ##
        inp_dpo = inp.repeat(2, 1, 1, 1, 1)
        # dpo_output ##
        tar_dpo = torch.cat((win, lose), dim=0)
        ...
        if (step+1) % self.update_interval == 0:
            self.ref_model.load_state_dict(self.model[list(self.model.keys())[0]].state_dict())
            for param in self.ref_model.parameters():
                param.requires_grad = False
            self.logger.info(f"Step {step}: 已将主模型参数更新至参考模型。")
    ...
    def test_one_step(self, batch_data, epoch, idx):
        ...
        if idx % 10 == 0:
            self.visualizer.save_pixel_image_4(pred_image=z_sample_prediction, target_img=tar, ref=z_sample_prediction_ref, step=epoch, batchidx=idx,
                                               predcsi=pred_csi, predfar=pred_far, refcsi=ref_csi, reffar=ref_far)
    ...
    def test(self, test_data_loader, epoch, step, state='Train'):
        ...
        for batch_idx, batch in enumerate(tqdm(test_data_loader, desc="Processing", total=total_step)):
