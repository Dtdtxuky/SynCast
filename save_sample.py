...
def subprocess_fn(args):
    ...
    # model_without_ddp.load_checkpoint(ckpt_path, load_model=True, load_optimizer=False, load_scheduler=False, load_epoch=True, load_metric_best=True)
    ...
    path_guidance = '/mnt/petrelfs/xukaiyi/CodeSpace/rankcast/test/guidance/guidance-scale0.005.txt'
    # model_without_ddp.save_sample(dataloader=save_dataloader, split=split)
    # model_without_ddp.rank_by_sample_guidance(txt_path=path_guidance)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg', '-c',                  type = str,     default = '/mnt/petrelfs/xukaiyi/CodeSpace/rankcast/configs/DiffCast/tau_sevir_ddpm.yaml',      help = 'path to the configuration file')
